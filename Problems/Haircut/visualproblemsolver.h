...
typedef QVector<HaircutCase> VectorCases;

QString SolveCase(const HaircutCase & theCase);
void AppendResult(HaircutCase & theCase, const QString & resultCase);

class Haircut : public QWidget
{
	Q_OBJECT

public:
	Haircut(QWidget *parent = 0, Qt::WindowFlags flags = 0);
