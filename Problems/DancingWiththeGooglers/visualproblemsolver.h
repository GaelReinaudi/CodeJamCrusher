...
typedef QVector<DancingWiththeGooglersCase> VectorCases;

QString SolveCase(const DancingWiththeGooglersCase & theCase);
void AppendResult(DancingWiththeGooglersCase & theCase, const QString & resultCase);

class DancingWiththeGooglers : public QWidget
{
	Q_OBJECT

public:
	DancingWiththeGooglers(QWidget *parent = 0, Qt::WFlags flags = 0);
...
private:
	Ui::DancingWiththeGooglersClass ui;
};
