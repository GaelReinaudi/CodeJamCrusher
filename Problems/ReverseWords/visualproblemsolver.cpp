...
ReverseWords::ReverseWords(QWidget *parent, Qt::WFlags flags)
	...
	, m_ShowFileContents(true)
{
	ui.setupUi(this);
	// reload settings
	QSettings settings("CodeJam", "ReverseWords");
...
void ReverseWords::MultiThreadedCompute(VectorCases & allCases)
{
...
QString ReverseWords::MakeSampleInput( QString strFromSampleDrop )
{
...
VectorCases ReverseWords::ParseInput(QTextStream & inputStream)
{
...
QString ReverseWords::MakeFileInOut( QString pathInput  /*= ""*/)
{
	QString content;
	m_FileIn.close();
...
void ReverseWords::CompareBoxesOutput()
{
	bool passes = false;
...
void ReverseWords::EventFinishedComputation()
{
	m_FileIn.close();
	m_FileOut.close();
	QString content;
