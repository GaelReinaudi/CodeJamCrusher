...
typedef QVector<TicTacToeTomekCase> VectorCases;

QString SolveCase(const TicTacToeTomekCase & theCase);
void AppendResult(TicTacToeTomekCase & theCase, const QString & resultCase);

class TicTacToeTomek : public QWidget
{
	Q_OBJECT

public:
	TicTacToeTomek(QWidget *parent = 0, Qt::WFlags flags = 0);
