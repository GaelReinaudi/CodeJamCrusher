...
typedef QVector<SquareTilesCase> VectorCases;

QString SolveCase(const SquareTilesCase & theCase);
void AppendResult(SquareTilesCase & theCase, const QString & resultCase);

class SquareTiles : public QWidget
{
	Q_OBJECT

public:
	SquareTiles(QWidget *parent = 0, Qt::WFlags flags = 0);
