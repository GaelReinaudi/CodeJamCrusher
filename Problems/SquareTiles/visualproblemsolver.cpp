...
SquareTiles::SquareTiles(QWidget *parent, Qt::WFlags flags)
	...
	, m_ShowFileContents(true)
{
	ui.setupUi(this);
	// reload settings
	QSettings settings("CodeJam", "SquareTiles");
...
void SquareTiles::MultiThreadedCompute(VectorCases & allCases)
{
...
void SquareTiles::SequencialCompute( VectorCases & allCases )
{
	foreach(SquareTilesCase theCase, allCases) {
...
void SquareTiles::dropEvent( QDropEvent* event )
{
...
QString SquareTiles::MakeSampleInput( QString strFromSampleDrop )
{
...
VectorCases SquareTiles::ParseInput(QTextStream & inputStream)
{
...
QString SquareTiles::MakeFileInOut( QString pathInput  /*= ""*/)
{
	QString content;
	m_FileIn.close();
	QString fileOutName;
...
void SquareTiles::CompareBoxesOutput()
{
	bool passes = false;
...
void SquareTiles::EventFinishedComputation()
{
	m_FileIn.close();
	m_FileOut.close();
	QString content;
...
void AppendResult( SquareTilesCase & theCase, const QString & resultCase )
{
