...
typedef QVector<PasswordProblemCase> VectorCases;

QString SolveCase(const PasswordProblemCase & theCase);
void AppendResult(PasswordProblemCase & theCase, const QString & resultCase);

class PasswordProblem : public QWidget
{
	Q_OBJECT

public:
	PasswordProblem(QWidget *parent = 0, Qt::WFlags flags = 0);
