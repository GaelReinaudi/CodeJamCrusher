...
HallofMirrors::HallofMirrors(QWidget *parent, Qt::WFlags flags)
	...
	, m_ShowFileContents(true)
{
	ui.setupUi(this);
	// reload settings
	QSettings settings("CodeJam", "HallofMirrors");
...
void HallofMirrors::MultiThreadedCompute(VectorCases & allCases)
{
...
void HallofMirrors::SequencialCompute( VectorCases & allCases )
{
	foreach(HallofMirrorsCase theCase, allCases) {
...
void HallofMirrors::dropEvent( QDropEvent* event )
{
...
QString HallofMirrors::MakeSampleInput( QString strFromSampleDrop )
{
...
VectorCases HallofMirrors::ParseInput(QTextStream & inputStream)
{
...
QString HallofMirrors::MakeFileInOut( QString pathInput  /*= ""*/)
{
	QString content;
	m_FileIn.close();
	QString fileOutName;
...
void HallofMirrors::CompareBoxesOutput()
{
	bool passes = false;
...
void HallofMirrors::EventFinishedComputation()
{
	m_FileIn.close();
	m_FileOut.close();
	QString content;
...
void AppendResult( HallofMirrorsCase & theCase, const QString & resultCase )
{
