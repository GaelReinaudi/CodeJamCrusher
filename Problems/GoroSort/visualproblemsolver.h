...
typedef QVector<GoroSortCase> VectorCases;

QString SolveCase(const GoroSortCase & theCase);
void AppendResult(GoroSortCase & theCase, const QString & resultCase);

class GoroSort : public QWidget
{
	Q_OBJECT

public:
	GoroSort(QWidget *parent = 0, Qt::WFlags flags = 0);
