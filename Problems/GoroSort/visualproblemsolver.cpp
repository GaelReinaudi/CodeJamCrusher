...
#include "GoroSortCase.h"

QFile m_FileIn;
QFile m_FileOut;

GoroSort::GoroSort(QWidget *parent, Qt::WFlags flags)
	...
	, m_ShowFileContents(true)
{
	ui.setupUi(this);
	// reload settings
	QSettings settings("CodeJam", "GoroSort");
...
void GoroSort::MultiThreadedCompute(VectorCases & allCases)
{
...
void GoroSort::SequencialCompute( VectorCases & allCases )
{
	foreach(GoroSortCase theCase, allCases) {
...
void GoroSort::dropEvent( QDropEvent* event )
{
...
QString GoroSort::MakeSampleInput( QString strFromSampleDrop )
{
...
VectorCases GoroSort::ParseInput(QTextStream & inputStream)
{
...
QString GoroSort::MakeFileInOut( QString pathInput  /*= ""*/)
{
	QString content;
	m_FileIn.close();
	QString fileOutName;
...
void GoroSort::CompareBoxesOutput()
{
	bool passes = false;
...
void GoroSort::EventFinishedComputation()
{
	m_FileIn.close();
	m_FileOut.close();
	QString content;
...
void AppendResult( GoroSortCase & theCase, const QString & resultCase )
{
