...
typedef QVector<NewLotteryGameCase> VectorCases;

QString SolveCase(const NewLotteryGameCase & theCase);
void AppendResult(NewLotteryGameCase & theCase, const QString & resultCase);

class NewLotteryGame : public QWidget
{
	Q_OBJECT

public:
	NewLotteryGame(QWidget *parent = 0, Qt::WindowFlags flags = 0);
