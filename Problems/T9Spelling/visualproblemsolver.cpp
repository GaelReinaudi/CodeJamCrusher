...
T9Spelling::T9Spelling(QWidget *parent, Qt::WFlags flags)
	...
	, m_ShowFileContents(true)
{
	ui.setupUi(this);
	// reload settings
	QSettings settings("CodeJam", "T9Spelling");
...
void T9Spelling::MultiThreadedCompute(VectorCases & allCases)
{
...
void T9Spelling::SequencialCompute( VectorCases & allCases )
{
...
void T9Spelling::dropEvent( QDropEvent* event )
{
...
QString T9Spelling::MakeSampleInput( QString strFromSampleDrop )
{
...
VectorCases T9Spelling::ParseInput(QTextStream & inputStream)
{
...
QString T9Spelling::MakeFileInOut( QString pathInput  /*= ""*/)
{
	QString content;
	m_FileIn.close();
	if(pathInput.isEmpty())
		...
		if(m_FileIn.fileName().contains("small"))
			m_FileOut.setFileName("../Outputs/T9Spelling-Small.txt");
		else if(m_FileIn.fileName().contains("large"))
			m_FileOut.setFileName("../Outputs/T9Spelling-Large.txt");
		else
			m_FileOut.setFileName("../Outputs/T9Spelling.txt");
...
void T9Spelling::CompareBoxesOutput()
{
	bool passes = false;
...
void T9Spelling::EventFinishedComputation()
{
	m_FileIn.close();
	m_FileOut.close();
	QString content;
