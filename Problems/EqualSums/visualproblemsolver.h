...
typedef QVector<EqualSumsCase> VectorCases;

QString SolveCase(const EqualSumsCase & theCase);
void AppendResult(EqualSumsCase & theCase, const QString & resultCase);

class EqualSums : public QWidget
{
	Q_OBJECT

public:
	EqualSums(QWidget *parent = 0, Qt::WFlags flags = 0);
