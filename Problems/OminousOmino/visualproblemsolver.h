...
class OminousOminoCase;

typedef QVector<OminousOminoCase> VectorCases;

QString SolveCase(const OminousOminoCase & theCase);
void AppendResult(OminousOminoCase & theCase, const QString & resultCase);

class OminousOmino : public QWidget
{
	Q_OBJECT

public:
	OminousOmino(QWidget *parent = 0, Qt::WindowFlags flags = 0);
