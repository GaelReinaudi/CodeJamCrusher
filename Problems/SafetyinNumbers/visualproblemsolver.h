...
typedef QVector<SafetyinNumbersCase> VectorCases;

QString SolveCase(const SafetyinNumbersCase & theCase);
void AppendResult(SafetyinNumbersCase & theCase, const QString & resultCase);

class SafetyinNumbers : public QWidget
{
	Q_OBJECT

public:
	SafetyinNumbers(QWidget *parent = 0, Qt::WFlags flags = 0);
